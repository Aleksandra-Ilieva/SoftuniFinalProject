...
### Страница с Цени
Ценовата страница може да бъде достъпена от всеки потребител ,включително нерегистриран. В тази страница се показват цените на услугите, които се съхраняват в базата данни. При стартиране на приложението се чете CSV файл, който се намира в ресурсите на проекта, за да се заредят цените. Цените са посочени в долари, като за трансформацията им от долари в евро се използва API, споменато по-рано: **[API за валутните курсове](https://www.exchangerate-api.com/)**.
...
Потребителят може да се регистрира в сайта чрез следната форма. Валидациите се извършват както на Frontend, така и на Backend ниво.
...
- Валидира коректността на имейла.
- Проверява дали вече няма съществуващ регистриран имейл или потребителско име.
- Проверява за нулева стойност или празно поле за всяко поле.
- Проверява дали паролата и потвърдената парола съвпадат.
...
След регистрация, потребителят бива пренасочен към формата за вход, чрез която може да влезе с имейл и парола.

![alt text](src/main/resources/static/readmeImg/login.png)
#### Аутентикация с Spring Security

Аутентикацията на потребителя се извършва с помощта на Spring Security, конфигуриран в `src/main/java/org/example/softunifinalproject/config/SecurityConfig.java`. При грешна парола или имейл се получава следното съобщение:

![alt text](src/main/resources/static/readmeImg/login2.png)

#### Регистрация на Администраторски Потребител

При стартиране на приложението, в базата данни се регистрира един потребител с администраторски права, който получава всички роли. Данните за този администратор и ролите се задават в `src/main/java/org/example/softunifinalproject/init/DbInitAdminAndRoles.java`. Тази конфигурация осигурява запазването на администратора и всички роли, ако вече не са били създадени.
...
Ако регистрирате нов потребител през сайта, той автоматично получава само роля `USER`. За демонстрация, потребител с имейл `aleks_5@abv.bg` ще бъде регистриран с роля `USER`.
...
Администраторът може да регистрира нови потребители и да им задава роли. За да се зададе роля на нов потребител, администраторът трябва да въведе името и имейла на потребителя и да избере желаната роля. След натискане на бутона за запазване, ролята е успешно зададена на потребителя.
...
При опит за задаване на роля, която вече съществува, се показва съответното съобщение за грешка.
...
## Вход като Нормален Потребител
Нормалният потребител може да запази час. Процесът на резервация включва следните валидации, извършвани чрез анотации и JavaScript:
...
След успешното изпращане на заявка за час, доктора получава известие и има възможност да одобри или откаже запазването.
![alt text](src/main/resources/static/readmeImg/user4.png)

## Вход като Доктор
лед като потребителят е изпратил заявка за час, докторът може да я одобри или откаже. Потребителят ще получи автоматичен имейл с уведомление за състоянието на заявката.
...
## Вход като Доктор
При клик на бутона „Консултиран“, записът се маркира като консултиран и изчезва от интерфейса, но не се изтрива веднага от базата данни. Записът остава в базата данни и се изтрива автоматично на всеки два часа с помощта на scheduler.
...
- **Форма за Запитване от Гости**: Всеки потребител, дори и нерегистриран, може да изпрати запитване, след като попълни формата.
![alt text](src/main/resources/static/readmeImg/contactUs.png)

- **Автоматично Попълване на Имейл**: Ако потребителят има акаунт и е влезнал, неговият имейл адрес се попълва автоматично в съответното поле на формата.
...
При натискане на бутона за смяна на езика, всяка страница се превежда на избрания език. Промяната на езика и актуализацията на бутона се осъществява чрез JavaScript.

![alt text](src/main/resources/static/readmeImg/internationalization.png)

### Responsive
Всички страници са респонсивни, т.е. автоматично се адаптират към различни размери на екрана. Падащото меню, което се отваря при клик, е реализирано с помощта на JavaScript.
